import { IOptionsMatch } from '@/components';
...
function drawTeamPlayers(
	...
	for (let i = 0; i < players.length; i++) {
		...
		if (optionsMatch) {
			...
			if (optionsMatch?.isShowNumber)
				ctx.fillText(String(players[i].number), 0, 0); // in the middle
			if (optionsMatch?.isShowFitness)
				ctx.fillText(players[i].fitness.toFixed() + '%', 0, radiusItems + 20);
		}
		ctx.restore();

		const isChangedOnPlayer =
			replacements && replacements.find(p => p.on === player._id);
