...
const OwnUserTeamComponent = (): JSX.Element => {
	...
	const formik = useFormik<IOwnUserTeamInitialValues>({
		...
		onSubmit: async (values, formik) => {
			...
			// check on the new club logo
			if (values.userTeamImgField)
				formData.append('userTeamImgField', values.userTeamImgField);

			// check on the new club name
			const isNewClubName = initVersionTeam.clubName !== values.clubName;
			if (isNewClubName) formData.append('clubName', values.clubName);
	...
	return (
		<>
			{/* modal for confirmation of deleting team */}
			<GlobalModal
